...
poplar::Tensor zerosLike(poplar::Graph &g, poplar::Tensor t,
...
adjustIndicesAndCreateMasks(
    ...
    std::vector<std::size_t> &partitionSizes, poplar::Tensor indices,
  ...
  for (auto s = 0; s < vGraphs.size(); s++) {
    ...
    popops::subInPlace(vGraph, unsignedIndices[s], partitionStart, prog,
    ...
    auto mask = popops::map(vGraph, betweenExpr, operands, prog,
...
poplar::Tensor
copyToAll(poplar::program::Sequence &prog, poplar::Graph &graph,
          std::vector<std::reference_wrapper<poplar::Graph>> vGraphs,
          poplar::Tensor input, int64_t inputIpu,
...
std::vector<poplar::Tensor> CrossEntropyShardedWROpx::sharded_log_softmax(
  ...
  for (auto i = 0; i < numShards; i++) {
    auto max =
        popops::reduce(vGraphs[i], logits[i], {1},
                       popops::ReduceParams(popops::Operation::MAX, false),
                       prog, "/reduce_partial_max");
  ...
  auto max = gcl::allReduceWithinReplica(graph, interIpuMaxs,
                                         gcl::CollectiveOperator::MAX, prog,
                                         "all_reduce_max");
  ...
  for (auto i = 0; i < numShards; i++) {
    ...
    auto partialDenominator =
        popops::reduce(vGraphs[i], numerators, numerators.elementType(), {1},
                       popops::Operation::ADD, prog, "reduce_partial_denom");
  ...
  for (int i = 0; i < shardedTranslated.size(); i++) {
    ...
    auto logsum =
        popops::map(vGraphs[i],
                    ...
                    {denomShard}, prog, "log_denominator");
    popops::subInPlace(vGraphs[i], shardedTranslated[i], logsum.expand({1}),
                       prog, "subtract_logsum");
  }
...
poplar::Tensor CrossEntropyShardedWROpx::sharded_take_last(
    poplar::Graph &graph,
    std::vector<std::reference_wrapper<poplar::Graph>> vGraphs,
    poplar::program::Sequence &prog, std::vector<poplar::Tensor> negLogSoftmax,
  ...
  for (auto s = 0; s < numShards; s++) {
    ...
    for (auto r = 0u; r < shardInput.dim(0); ++r) {
      ...
      auto scalar = popops::multiSlice(
          vGraphs[s], sliceableRow, rowIndex, {0}, {1}, prog, {}, {},
          "take_along_row_" + rowSuffix + "_shard_" + shardSuffixStr);
    ...
    popops::selectInPlace(vGraphs[s], shardSlice, zeros, mask, prog,
  ...
  auto result =
      popops::reduce(graph, partials, partials.elementType(), {0},
                     popops::Operation::ADD, prog, "reduce_partial_denom");
...
void CrossEntropyShardedWROpx::grow(poplar::program::Sequence &prog) const {
  ...
  for (int i = 0; i < logSoftmaxSharded.size(); i++) {
    popops::negInPlace(graphTop, logSoftmaxSharded[i], progPop,
                       "/neg_log_softmax" + std::to_string(i));
  }

  auto loss = sharded_take_last(graphTop, vGraphs, progPop, logSoftmaxSharded,
                                indiciesConcat);
...
void CrossEntropyShardedGradWROpx::grow(poplar::program::Sequence &prog) const {
  ...
  auto gradientShardedCopies = copyToAll(progPop, graphTop, vGraphs, gradient,
                                         ipus[0], "/broadcast_grad_to_shards");
  ...
  for (auto i = 0u; i < numShards; ++i) {
    ...
    popops::mulInPlace(vGraphs[i], softmax, scale, progPop,
                       "/scale_softmax_acts");
  ...
  for (auto c = 0u; c < vGraphs.size(); ++c) {
    ...
    for (auto i = 0u; i < activation.dim(0); ++i) {
      ...
      popops::multiUpdateAdd(
          vGraphs[c], u_, g_, a_, minusOne, {0}, {1}, progPop, {}, {},
          "/scatter_gradients_" + shardSuffixStr + "_" + std::to_string(i));
