...
public:
  ...
  void setup() override {
    outInfo(0) = TensorInfo(DataType::FLOAT, Shape{});
  }
...
public:
  GradReduceSquareAddOpx(Op *op, Devicex *devicex): PopOpx(op, devicex) {
    verifyOp<GradReduceSquareAddOp>(op, {GradReduceSquareAddOp::defaultOperatorId()});
  }
...
popx::OpxCreator<GradReduceSquareAddOpx> GradReduceSquareAddOpxCreator(GradReduceSquareAddOp::defaultOperatorId());
...
PYBIND11_MODULE(grad_reduce_square_add_binding, m) {
  ...
  binding.def(py::init<const popart::Op::Settings &>(),
              py::arg("settings"));
