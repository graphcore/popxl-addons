...
Tensor maskedAllGatherCopy(Graph &graph, const Tensor &data,
  ...
  popops::mapInPlace(graph, popops::expr::_2 % stride, {groupID, replicaIndex},
                     prog, {"computeGroupID"});
...
Tensor maskedAllGather(Graph &graph, const Tensor &data,
  ...
  Tensor offsets_1 = graph.addConstant(poplar::UNSIGNED_INT, {group, 1},
  ...
  Tensor res = popops::multiSlice(
      graph, temp.reshape({group * stride, data.numElements()}), offsets,
      {0, 1}, {1, data.numElements()}, prog, {}, {}, {"multiCopy"});
  return res.reshape(shape);
}

Tensor maskedAllGatherConcat(Graph &graph, const Tensor &data,
                             program::Sequence &prog, uint32_t stride,
                             uint32_t group) {
  gcl::CommGroup cgroup{gcl::CommGroupType::CONSECUTIVE, group * stride};
  Tensor temp = gcl::allGatherCrossReplica(graph, data, prog, cgroup);
...
Tensor maskedAllReduce(Graph &graph, const Tensor &data,
  ...
  poplar::Tensor sum =
      popops::reduce(graph, res.reshape({group, data.numElements()}),
                     data.elementType(), {0}, params, prog, {"sum"});
  return sum.reshape(data.shape());
}

Tensor maskedAllReduceConcat(Graph &graph, const Tensor &data,
                             program::Sequence &prog, uint32_t stride,
                             uint32_t group) {
  gcl::CommGroup cgroup{gcl::CommGroupType::CONSECUTIVE, group * stride};
  Tensor temp = gcl::allGatherCrossReplica(graph, data, prog, cgroup);
...
Tensor maskedReduceScatter(Graph &graph, const Tensor &data,
  ...
  Tensor size = popops::map(graph, popops::expr::_1 - popops::expr::_2,
                            {end, start}, prog, {"size"});
  ...
  for (uint32_t s : sizes) {
    ...
    if (s != 0) {
      popops::dynamicSliceWithOutput(graph, scatter.slice(0, s), sum,
...
void allReduceInit(const Tensor &data, program::Sequence &prog,
                   const Tensor &accum, const Tensor &src, const Tensor &src2) {
...
program::Sequence
allReduceLoop(Graph &graph, const Tensor &accum, const Tensor &src,
              const Tensor &src2, const Tensor &dest, const Tensor &dest2,
              const Tensor &step,
  ...
  Tensor strideT = popops::map(graph, popops::expr::_1 % halfStride, {step},
                               loop, {"strideIndex"});
  std::vector<std::pair<std::int32_t, program::Program>> switchBody;
...
Tensor ringAllReduce(Graph &graph, const Tensor &data, program::Sequence &prog,
                     uint32_t stride, uint32_t group) {
  ...
  program::Sequence loop = allReduceLoop(graph, accum, src, src2, dst, dst2,
                                         step, ring, revRing, stride, true);
  prog.add(program::Repeat((ring.size() / 4) - 1, loop, {"loop"}));
  program::Sequence endLoop = allReduceLoop(graph, accum, src, src2, dst, dst2,
                                            step, ring, revRing, stride, false);
  prog.add(endLoop);
  return accum;
}

Tensor ringReduceScatter(Graph &graph, const Tensor &data,
                         program::Sequence &prog, uint32_t stride,
                         uint32_t group) {
...
void allGatherInit(Graph &graph, const Tensor &data, program::Sequence &prog,
                   const Tensor &res, const Tensor &index, const Tensor &src,
                   const Tensor &src2) {
  prog.add(program::Copy(data, src));
  prog.add(program::Copy(data, src2));
  popops::dynamicUpdate(graph, res, data.expand({0}), index.expand({0}), {0},
                        {1}, prog, {"initialCopy"});
}

program::Sequence
allGatherLoop(Graph &graph, const Tensor &res, const Tensor &src,
              const Tensor &src2, const Tensor &dest, const Tensor &dest2,
              const Tensor &step, const Tensor &index1, const Tensor &index2,
              const std::map<unsigned int, unsigned int> &ring,
              const std::map<unsigned int, unsigned int> &revRing,
              uint32_t stride, uint32_t group, bool full) {
  program::Sequence loop;
  loop.add(program::CrossReplicaCopy(src, dest, ring, {"CrossReplica1"}));
  if (full)
    loop.add(
  ...
  program::Sequence copies;
  if (full) {
    popops::mapInPlace(graph, (popops::expr::_1 + 1) % group, {index2}, copies,
                       {"decrementIndex2"});
    popops::dynamicUpdate(graph, res, dest2.expand({0}), index2.expand({0}),
                          {0}, {1}, copies, {"copyBackward"});
  }
  popops::mapInPlace(graph, (popops::expr::_1 - 1) % group, {index1}, copies,
                     {"indcrementIndex1"});
  popops::dynamicUpdate(graph, res, dest.expand({0}), index1.expand({0}), {0},
                        {1}, copies, {"copyForward"});
...
Tensor ringAllGather(Graph &graph, const Tensor &data, program::Sequence &prog,
                     uint32_t stride, uint32_t group) {
  std::map<unsigned int, unsigned int> ring = createRing(graph, stride, group),
                                       revRing = reverseMap(ring);
  ...
  Tensor replicaIndex = graph.addReplicationIndexConstant({"replicaIndex"});
  graph.setTileMapping(replicaIndex, 1);
  popops::mapInPlace(graph, (popops::expr::_2 / stride), {index1, replicaIndex},
                     prog, {"computeIndex"});
  ...
  prog.add(program::Copy(one, step));
  program::Sequence loop =
      allGatherLoop(graph, res, src, src2, dst, dst2, step, index1, index2,
                    ring, revRing, stride, group, true);
  prog.add(program::Repeat((ring.size() / 4) - 1, loop, {"loop"}));
  program::Sequence endLoop =
      allGatherLoop(graph, res, src, src2, dst, dst2, step, index1, index2,
                    ring, revRing, stride, group, false);
  prog.add(endLoop);
  return res;
}
