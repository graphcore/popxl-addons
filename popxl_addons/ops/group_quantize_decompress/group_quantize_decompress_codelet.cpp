...
//
half4 convert4(unsigned short x) {
...
//
void convert4_vec(const unsigned short *x, half4 *y, unsigned n) {
  for (auto i = 0u; i < n; ++i) {
    y[i] = convert4(x[i]);
  }
}
...
struct DecompressPacked4BitTensor : poplar::Vertex {
  poplar::Input<poplar::Vector<uint16_t, poplar::VectorLayout::SPAN>> input;
