...
def cross_entropy(
    x: popxl.Tensor, labels: popxl.Tensor, ignore_index: Optional[int] = None, reduction: REDUCTION_TYPE = "mean"
) -> popxl.Tensor:
    """
...
def cross_entropy_with_grad(
    ...
    loss, dx = ops.nll_loss_with_softmax_grad(
        probs, labels, loss_grad=loss_scaling, ignore_index=ignore_index, reduction=reduction
    )
    return loss, dx
