...
__all__ = ["all_gather_replica_sharded_graph", "reduce_replica_sharded_graph", "replica_sharded_slice_graph"]
...
def replica_sharded_slice_graph(tensors: NamedTensors, threshold: int = 1024,
                                use_io_tiles: bool = False) -> Tuple[GraphWithNamedArgs, List[str]]:
    ...
    with graph, popxl.in_sequence(False), tile_context:
        for name, tensor in zip(*tensors.unpack()):
            ...
            if sg_t.nelms >= threshold and sg_t.nelms % graph.ir.replication_factor == 0:
                sg_t = ops.collectives.replica_sharded_slice(sg_t)
