...
from popxl.tensor import Variable, remote_replica_sharded_variable, remote_variable
...
class RemoteBuffers:
    ...
    def __init__(self):
        self.buffers: Dict[int, popxl.RemoteBuffer] = {}
        self.hash_to_id: Dict[int, int] = {}

    def get_buffer(self, shape: Iterable[int], dtype: popxl.dtype,
                   meta_shape: Optional[Iterable[int]] = None) -> popxl.RemoteBuffer:
        ...
        Args:
            ...
            dtype (popxl.dtype): Datatype of the entries of the Buffer
        ...
        if buffer_hash not in self.hash_to_id:
            buffer = popxl.remote_buffer(tuple(shape), dtype, 1)
    ...
    def remote_variable(self, var: Variable) -> Tuple[popxl.RemoteBuffer, int]:
        ...
        Returns:
            Tuple[popxl.RemoteBuffer, int]: RemoteBuffer and offset of the remote variable storage
        """
    ...
    def replica_sharded_variable(self, var: Variable) -> Tuple[popxl.RemoteBuffer, int]:
        ...
        Returns:
            Tuple[popxl.RemoteBuffer, int]: RemoteBuffer and offset of the remote variable storage
        """
        shape = (int(np.prod(var.shape)) // popxl.gcg().ir._pb_ir.getSessionOptions().replicatedGraphCount, )
