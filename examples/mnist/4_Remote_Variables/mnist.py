...
def remote_step(var: popxl.Tensor, grad: popxl.Tensor, optimizer: addons.Module, opts):
    facts, opt_graph = optimizer.create_graph(replica_sharded_spec(var, threshold=opts.sharded_threshold),
                                              replica_sharded_spec(grad, threshold=opts.sharded_threshold),
                                              lr=opts.lr)

    # keep the state of the optimizer in remote buffers
    buffers = addons.named_variable_buffers(facts, entries=1, sharded_threshold=opts.sharded_threshold)
...
def optimizer_step(variables: NamedTensors, grads: Dict[popxl.Tensor, popxl.Tensor], optimizer: addons.Module,
                   accum_counter: popxl.Tensor, opts):
    ...
    with ir.main_graph:
        ...
        with popxl.in_sequence(True):
            ...
            if opts.data_parallel > 1:
                ...
                grad_reduce, names = reduce_replica_sharded_graph(grads, 'mean', threshold=opts.sharded_threshold)
...
def main():
    ...
    parser.add_argument('--data-parallel', type=int, default=4, help='data parallelism (default: 2)')
    parser.add_argument('--gradient-accumulation', type=int, default=8, help='gradient accumulation (default: 4)')
